...
def tv_eval(dataset, pnp_type, kernel_size, num_pictures, num_iter, step_size, max_cgiter, cg_tol, blur_type, angle, blur_kernel_size): 
    # mkdir
    if not os.path.exists('./eval_dataset/'):
        os.makedirs('./eval_dataset/')
    if not os.path.exists(f"./eval_output_tv_{kernel_size}/"):
        os.makedirs(f"./eval_output_tv_{kernel_size}/")
    # Load Data
    ...
    for img_file in img_files:
        ...
        if pnp_type == 'pnp_admm_least_square': 
            ...
            denoised_image_bm3d = pnp_admm_cg(y, forward, forward_adjoint, tv_denoiser, step_size=step_size, num_iter=num_iter, max_cgiter=max_cgiter, cg_tol=cg_tol)
        ...
        output_path = f"./eval_output_tv_{kernel_size}/{img_file.split('/')[-1].replace('.jpg', '_denoised.jpg')}"
    ...
    file_path = f"eval_output_tv_{kernel_size}.txt"
